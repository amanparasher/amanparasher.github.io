---
permalink: /
# title: "Aman Parasher - Data Engineer"
author_profile: true
redirect_from: 
  - /about/
  - /about.html
---

<!-- Hello! I'm Aman Parasher.

Welcome to my portfolio. I'm a **B.Tech graduate** and a **Data Engineer Intern at Mactores**, where I build and manage real-world data pipelines. I am deeply focused on the **Databricks ecosystem** and am currently pursuing the Databricks Data Engineer Professional certification, having already cleared the Associate level.

My journey to this point was built on a foundation of intensive, hands-on learning during my B.Tech at SRMIST (2022-2025). This included a **3-month AI research internship in Taiwan** (specializing in Deep Learning and Computer Vision) and practical virtual internships with industry leaders like KPMG and TATA.

This hands-on engineering experience is complemented by my unique background in tech recruiting, giving me a rare perspective on how to build data solutions that deliver real business value. This portfolio is a showcase of that philosophy. Here you'll find end-to-end projects that solve real, recognizable business problems.

---

### **Explore My Portfolio**

To understand my progression and see all my certifications, please see my **[Tech Journey](/tech-journey/)**.

To see my major projects (including the Uber Surge Pricing Pipeline and HFT Backtesting Engine on Databricks), please explore my **[Projects](/portfolio/)**.

To learn about my formal internships and research, you can visit my **[Experience section](/work-experience/)**.

To have a quick review of my whole portfolio, please see my **[CV/Resume](/cv/)**.

---
Please excuse any dead links or 404 errors; this website is a living document and constantly under construction as I build new things! -->





<!-- 2nd one -->

## # Hello! I'm Aman Parasher

## Engineering Scalable Data Infrastructure

I am a **B.Tech graduate** in Computational Intelligence from SRM University, Chennai, and currently a **Data Engineer Intern at Mactores**, where I build production data pipelines that process real-time and batch workloads at scale.

As a **Databricks Certified Data Engineer Associate**, I work extensively with **Apache Spark, Delta Lake, and the Databricks Lakehouse platform**. I am currently pursuing the Professional certification to advance my expertise in distributed data systems.

---

## Background

My technical journey started with a B.Tech in Computational Intelligence at SRM University (2021-2025), where I built strong foundations in computer science and AI systems.

Key experiences include:

* **AI Research Intern** at National Chung Cheng University, Taiwan (Taiwan India Joint Research Centre on AI)
* **Virtual Internships** with KPMG and Tata to gain hands-on experience with enterprise data workflow

---

## Explore My Work

### Projects & Experience $\rightarrow$

See my technical implementations including:

* **Uber Surge Pricing Pipeline Project:** real-time streaming using Kafka and Spark
* **HFT Backtesting Engine:** low-latency quantitative analysis on Databricks

**[View Projects and Experience Here](/portfolio/)**

### CV/Resume $\rightarrow$

Complete overview of my technical stack, certifications, and professional timeline

**[View/Download CV (PDF)](/files/Aman_Parasher_CV.pdf){:target="_blank"}**

---






<!-- 3rd one -->
<!-- 
## Aman Parasher — Data Engineer

Hello! I'm Aman Parasher.

I am a **Data Engineer Intern at Mactores** focused on designing, building, and managing real-world data pipelines within the **Databricks ecosystem**. I am currently working towards the Databricks Data Engineer Professional certification.

---

### **Impact-Driven Engineering**

My philosophy is rooted in engineering solutions that deliver clear business value. My background combines deep technical training with a unique perspective from tech recruiting, resulting in a project portfolio of **end-to-end solutions** that solve tangible business problems.

### **The Technical Foundation**

My journey includes:
* **B.Tech at SRMIST** (2022-2025)
* A **3-month AI research internship in Taiwan** (Deep Learning & Computer Vision).
* Practical virtual internships with industry leaders like KPMG and TATA.

---

### Where to Go Next

<div style="padding: 15px; border: 1px solid #ddd; border-left: 5px solid #007BFF; background-color: #f9f9f9;">
    <p style="margin-top: 0; font-size: 1.1em; font-weight: 600;">
        <i class="fas fa-clipboard-list" aria-hidden="true"></i> Dive into my work:
    </p>
    <ul style="list-style-type: none; padding-left: 0;">
        <li style="margin-bottom: 8px;">
            <i class="fas fa-code-branch" aria-hidden="true"></i> 
            <a href="/portfolio/" style="font-weight: bold; text-decoration: none;">
                View Major Projects &rarr;
            </a>
        </li>
        <li style="margin-bottom: 8px;">
            <i class="fas fa-user-tie" aria-hidden="true"></i> 
            <a href="/year-archive/" style="font-weight: bold; text-decoration: none;">
                Review Formal Internships & Experience &rarr;
            </a>
        </li>
        <li style="margin-bottom: 8px;">
            <i class="fas fa-file-alt" aria-hidden="true"></i> 
            <a href="/files/Aman_Parasher_CV.pdf/" style="font-weight: bold; text-decoration: none;">
                Download Full CV / Resume &rarr;
            </a>
        </li>
    </ul>
</div>
--- -->
